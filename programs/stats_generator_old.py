...
def hangman_stats_doc(sample, filename):
    header = ['word','tries','wrong','len_word','cont_unusual'] + [
        'cont_' + char for char in 'abcdefghijklmnopqrstuvwxyz']
    ...
    for i in range(len(sample)):
        word = sample[i]
        line_data = list(automatic_hangman(word)) + 
                    [contains_unusual_chars(word)] + 
                    contain_all_chars(word)
        all_lines.append(line_data)
        if i % 1000 == 0:
            print "making stats - on line:", i, " time elapsed:", time.time() - 
                start_time, "secs"
    print '\n'
    with open ('{filename}.csv'.format(filename=filename), 'wb') as stats_file:
        ...
        for i in range(len(all_lines)):
            ...
            if i % 1000 == 0:
                print "write to csv - on line", i, " time elapsed:", time.time() - 
                    start_time, "secs"
    print "Complete!"
